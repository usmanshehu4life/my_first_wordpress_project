 ...
 * @return array
 */
function educenter_body_classes( $classes ) {
	...
	if(is_singular(array( 'post','page' ))){
        ...
        $post_sidebar = get_post_meta($post->ID, 'educenter_page_layouts', true);
    ...
    if( is_home() ){
        ...
        $post_sidebar = get_post_meta( get_option( 'page_for_posts' ), 'educenter_page_layouts', true);
...
add_filter( 'body_class', 'educenter_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function educenter_pingback_header() {
...
add_action( 'wp_head', 'educenter_pingback_header' );


if(!function_exists('educenter_font_awesome_icon_array')){
	function educenter_font_awesome_icon_array(){
 ...
 */
if(!function_exists('get_educenter_alignment_class')){
    function get_educenter_alignment_class($align){
        if($align){
...
if(!function_exists('educenter_header_button')){
    function educenter_header_button(){
        $educenter_header_button_enable = get_theme_mod('educenter_header_button_enable', 'disable');
        
        if( $educenter_header_button_enable == 'enable' ){ 
            $educenter_hb_title = get_theme_mod('educenter_hb_title',esc_html__('Call Anytime 24/7', 'educenter'));
            $educenter_hb_text  = get_theme_mod('educenter_hb_text',esc_html__('+01-559-236-8009', 'educenter'));
            $educenter_hb_icon  = get_theme_mod('educenter_hb_icon', 'fa fa-phone-volume');
            $educenter_hb_link  = get_theme_mod('educenter_hb_link', esc_html__('tel:5592368009', 'educenter'));
            $header_btn_class = get_theme_mod('educenter_header_button_hide_show', json_encode(array(
                'desktop' => 'show',
                'tablet' => 'hide',
                'mobile' => 'hide'
            )));

            $button_class = get_educenter_alignment_class($header_btn_class);
            
            ob_start();
            ?>
            <div class="quickcontact">
                <div class="get-tuch educenter-header-button <?php echo $button_class; ?>">
                    <?php if( $educenter_hb_link): ?>
                        <a href="<?php echo esc_url( $educenter_hb_link ); ?>">
                    <?php endif; ?>
                        
                        <?php if( $educenter_hb_icon ): ?>
                            <i class="<?php echo esc_attr($educenter_hb_icon); ?>"></i>
                        <?php endif; ?>
                    ...
                    <div class="quickcontactwrap buttonwrap">
                        <?php if( $educenter_hb_title): ?>
                            <div class="quickcontact-title">
                                <?php echo esc_html( $educenter_hb_title ); ?>
                            </div>
                        <?php endif; ?>
                        <?php if( $educenter_hb_text): ?>
                            <span>
                                <?php echo esc_html( $educenter_hb_text ); ?>
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            return '<a class="btn btn-primary educenter-header-button'.$button_class.'" href="'.esc_attr($educenter_hb_link).'">'.wp_kses_post($educenter_hb_text).'</a>';
        }
    }
}
add_action('educenter_header_button', 'educenter_header_button');

if( !function_exists ('educenter_nav_buttons')){
	/** 
	 * Adding Search and Sidebar Navigation Items
	 */
	function educenter_nav_buttons() {
		$items = "";
		$educenter_header_button_enable = get_theme_mod('educenter_header_button_enable', 'disable');
		if( $educenter_header_button_enable == 'enable' ){
            $items = "<div class='nav-buttons'>";
			$items .= '<span class="menu-item-button no-hover">'. educenter_header_button().'</span>';
            $items .="</div>";
		}
	...
	add_action('educenter_nav_buttons', 'educenter_nav_buttons');
}

/*****
 * After Footer Section 
*/
if(!function_exists('educenter_add_footer_seperator')){
    function educenter_add_footer_seperator() {
        if( get_theme_mod('educenter_footer_section_seperator', 'no') == 'top'):
            
            $footer_top_seperator = get_theme_mod("educenter_footer_top_seperator", 'water-waves');
...
if(!function_exists('educenter_add_top_seperator')):

    function educenter_add_top_seperator($section_name) {

        $section_seperator = get_theme_mod("educenter_{$section_name}_section_seperator", "no");
        
        if ($section_seperator == 'top' || $section_seperator == 'top-bottom') {
            $top_seperator = get_theme_mod("educenter_{$section_name}_top_seperator", 'big-triangle-center');
...
if(!function_exists('educenter_add_bottom_seperator')):

    function educenter_add_bottom_seperator($section_name) {

        $section_seperator = get_theme_mod("educenter_{$section_name}_section_seperator", "no");

        if ($section_seperator == 'bottom' || $section_seperator == 'top-bottom') {

            $bottom_seperator = get_theme_mod("educenter_{$section_name}_bottom_seperator", 'big-triangle-center');
